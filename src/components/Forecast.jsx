...
function Forecast(props) {
  ...
  const classOne = "grow h-full flex justify-center items-center cursor-pointer text-white px-2 py-2";
  const classOneActive = "grow h-full flex justify-center items-center cursor-pointer text-black bg-white px-2 py-2";
  const classTwo = "grow h-full flex justify-center items-center cursor-pointer text-white px-2 py-2";
  const classTwoActive = "grow h-full flex justify-center items-center cursor-pointer text-black bg-white px-2 py-2";
  const classThree = "grow h-full flex justify-center items-center cursor-pointer text-white px-2 py-2";
  const classThreeActive = "grow h-full flex justify-center items-center cursor-pointer text-black bg-white px-2 py-2";
  return (<>
      ...
      <div className=" relative w-full h-[30rem] flex  items-center overflow-scroll">
        <div className="relative flex justify-center items-center w-full h-full min-h-[26rem]">
          <CardO data={active.hour[nowHours]} dte = {active.date} />
        </div>
        <div className="relative flex items-center w-full h-[20rem] pl-4 pr-4 rounded-xl  gap-4  lg:gap-8">
          <AirQuality data={active.hour[nowHours].air_quality} />
          <Wind data={active.hour[nowHours]} Id="foreWind" />
          <OtherStats data={active.hour[nowHours]} />
        </div>
      </div>
      <div className=" relative grow w-full h-64 max-w-6xl flex flex-col justify-center items-center max-md:pl-4 max-md:pr-4 pl-8 pr-8 glass rounded-xl mb-4">
        ...
        <img
          ...
          onClick={leftMove}
        />
        <Slider id="scroller" data={active.hour} nowPoint={nowHours}/>
        <img
          src={rightArw}
